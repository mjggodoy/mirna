...
public class InsertionOfMatureSequencesNotFromPlantDatabaseAndMirbase {
	...
	public InsertionOfMatureSequencesNotFromPlantDatabaseAndMirbase() throws IOException {
	...
	public void execute() throws Exception {
		...
		try {
			...
			String query = "select * from mirna_old.mirna_has_mature a, mirna_old.mature_has_sequence b, mirna_old.sequence c"
					+ " where a.mature_pk = b.mature_pk and b.sequence_pk = c.pk;";
			...
			while (rs.next() && limit!=0) {
				...
				if (list==null) {
					...
					//HAY QUE INSERTAR
					System.out.println("El tama–o de la lista es 0, hay que insertar secuencia: " + sequence + " " +mirna_pk);	
	...
	private List<MirnaResult> findDatabase(int mirna_pk) throws Exception {
		...
		try {
			...
			String query = "select * from mirna.sequence_mature where mirna_pk ="+ mirna_pk +";";
	...
	public static void main(String[] args) throws Exception {
		...
		InsertionOfMatureSequencesNotFromPlantDatabaseAndMirbase plants = new InsertionOfMatureSequencesNotFromPlantDatabaseAndMirbase();
		plants.execute();
